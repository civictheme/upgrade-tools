...
function fixImportPath(filepath, importpath) {
  if (importpath.includes('components')) {
    const pathArr = filepath.split('/').reverse()
    while(pathArr.length > 0 && pathArr[0] !== 'themes') {
      pathArr.shift()
    }
    const themeDir =  pathArr.reverse().join('/')

    const importArr = importpath.split('/')
    while(importArr.length > 0 && importArr[0] !== 'components') {
      importArr.shift()
    }
    const importFile =  importArr.join('/')

    return path.relative(filepath, themeDir) + '/themes/contrib/civictheme/' + importFile
  }
  return importpath
}
